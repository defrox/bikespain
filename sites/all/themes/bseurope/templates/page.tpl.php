...
?> 
 ">
<?php
$hideme = !in_array($node->nid, array(38,270,347,349,350,1648,1649));
if ($hideme):
?>
	<!-- TOP BAR 
	============================================= -->
...
$out = '';
$links = '';
if(!empty($node) && strcmp($node->type, 'productos')==0){
	foreach($page['content']['system_main']['nodes'] as $key=>$content){
		if(is_numeric($key)){
			$id=$key;
			break;
		}
	}
	if($page['content']['system_main']['nodes'][$id]['links']):
		$links = render($page['content']['system_main']['nodes'][$id]['links']);
	endif;
	
	$relacionados = $node->field_productos_relacionados;
	if(!empty($relacionados)){

		$tiposProductos = _taxonomy_options('tipo_producto', t('Search by trip - Any'));
		$out = '<div class="row">';
		$i = 1;
		foreach($relacionados['und'] as $fi){
        ...
        <div class="row-item col-1_4 sidebar">
          <?php print render($page['sidebar_first']); echo $links; ?>
        </div>
        <!-- // sidebar first -->
      <?php endif; ?>
